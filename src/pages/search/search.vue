<template>
  <page :better-scroll="true" page-background="white">
    ...
    <search-default
      :hot="search_index.hotKeywordList"
      :history="history"
...
<script type="text/ecmascript-6">
  ...
  export default {
    data() {
      return {
        step: 1,
        // searchKey: this.search_index.defaultKeyword.keyword,
        hot: [],
    ...
    computed: {
      ...mapState('search', ['search_index']),
      searchKey() {
        return this.search_index.defaultKeyword ? this.search_index.defaultKeyword.keyword : ''
      }
    },
    methods: {
      // ...mapActions('search', ['getSearchIndex', 'getSearchListBykeyword']),
      ...mapActions('search', ['getSearchIndex','getGoodsFilter']),
      change(key) {
      ...
      search(val) {
        ...
        if (val) {
          ...
          this.getGoodsFilter({keyword:val})
        } else {
